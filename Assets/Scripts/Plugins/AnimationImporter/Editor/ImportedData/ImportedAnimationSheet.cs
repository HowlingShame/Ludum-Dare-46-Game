		...
		*/
		...
		public void CreateAnimation(ImportedAnimation anim, string basePath, string masterName, AnimationTargetObjectType targetType)
		{
			...
			if (targetType == AnimationTargetObjectType.SpriteRenderer)
			{
				...
				AnimationUtility.SetObjectReferenceCurve(clip, AnimationClipUtility.imageCurveBinding(AnimationImporter.Instance.useFileNameAsPath ? masterName : AnimationImporter.Instance.objectPath), null);	// remove image curve
			}
			else if (targetType == AnimationTargetObjectType.Image)
			{
				AnimationUtility.SetObjectReferenceCurve(clip, AnimationClipUtility.spriteRendererCurveBinding(AnimationImporter.Instance.useFileNameAsPath ? masterName : AnimationImporter.Instance.objectPath), null);	// remove sprite curve
