...
public class StateMachine<TLabel>
{
	...
	private class State
	{
		...
		public readonly Func<TLabel, bool>	performTransition;

		public State(Action onStart, Action onUpdate, Action onStop, Action onReEnter, Func<TLabel, bool> performTransition)
		{
			this.onStart = onStart;
			this.onUpdate = onUpdate;
			this.onStop = onStop;
			this.onReEnter = onReEnter;
			this.performTransition = performTransition;
		}
	...
	public interface IStateObject
	{
		...
		void iOnUpdate();
		void iOnReEnter();
		bool iPerformTransition(TLabel label);
	}
	...
	public abstract class StateObject<TState> : IStateObject
	{
		...
		public virtual bool iPerformTransition(TLabel label) { return true; }
    ...
    /// <param name="hasTransition">Allow transition to the state function.</param>
    public void AddState(TLabel label, Action onStart = null, Action onUpdate = null, Action onStop = null, Action onReEnter = null, Func<TLabel, bool> hasTransition = null)
	{
        // add or override state
		stateDictionary[label] = new State(onStart, onUpdate, onStop, onReEnter ?? Empty, hasTransition ?? AlwaysPerformTransition);
	}
	...
	public void AddState<T>(T label) where T : IStateObject, TLabel
	{
        ...
        stateDictionary.Add(label, new State(label.iOnStart, label.iOnUpdate, label.iOnStop, label.iOnReEnter, label.iPerformTransition));
	}
	...
	{
		AddState(label, () => subMachine.implChangeState(subMachineStartState), subMachine.Update);
	}
	...
	/// </summary>
	private void implChangeState(TLabel stateLable)
	{
#if UNITY_EDITOR
		...
		if (m_CurrentState != null)
		{
			...
			if (m_CurrentState.performTransition(stateLable))
			{
